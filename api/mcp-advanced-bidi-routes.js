...
export function createAdvancedBidiRoutes(db, io) {
  ...
  router.post('/campaign/execute', async (req, res) => {
    try {
      ...
      const campaign = campaignRegistry.get(campaignId) || (await loadCampaignFromDB(campaignId));
    ...
    try {
      ...
      res.json({
        ...
        recommendations: result.rows.map(r => ({
          ...r,
          trust_level:
            r.success_rate > 0.9 ? 'excellent' : r.success_rate > 0.7 ? 'good' : 'acceptable',
        })),
      });
    } catch (error) {
      res.status(500).json({
  ...
  router.post('/orchestrate/multi-schema-task', async (req, res) => {
    try {
      ...
      // Load schemas
      const schemas = await db.query(`SELECT * FROM schemas WHERE id = ANY($1)`, [schemaIds]);
  ...
  router.post('/free-plan/engagement', async (req, res) => {
    try {
      ...
      res.json({
        ...
        insights,
        suggestions:
          insights.conversionPotential > 0.7
  ...
  function checkCampaignLimits(campaign) {
    if (
      campaign.limits.maxExecutions !== -1 &&
  ...
  async function updateCampaignUsage(campaignId, usage) {
    await db.query(`UPDATE campaigns SET usage = $2 WHERE id = $1`, [
      campaignId,
  ...
  async function stopCampaignResources(campaignId) {
    ...
    await db.query(`UPDATE mcp_stream_sessions SET active = false WHERE campaign_id = $1`, [
      campaignId,
    ]);
  }
  ...
  async function applyAutoConfig(serverId, config) {
    await db.query(`UPDATE mcp_servers SET config = $2 WHERE id = $1`, [
      serverId,
