...
class BlockchainTester {
  ...
  async testBlockchainStatus() {
    try {
      ...
      this.addResult('Blockchain Status', response.status === 200, 'Blockchain status endpoint working');
    } catch (error) {
  ...
  async testSubmitOptimization() {
    try {
      ...
      const response = await axios.post(`${this.apiUrl}/api/blockchain/submit-optimization`, testData);
      this.addResult('Submit Optimization', response.status === 200, 'Optimization submitted successfully');
    } catch (error) {
      this.addResult('Submit Optimization', false, `Optimization submission failed: ${error.message}`);
    }
  }
  ...
  async testMiningSession() {
    try {
      ...
      this.addResult('Start Mining Session', response.status === 200, 'Mining session started successfully');
      
      // Stop the session
      await axios.post(`${this.apiUrl}/api/blockchain/stop-mining`, { userAddress: testData.userAddress });
