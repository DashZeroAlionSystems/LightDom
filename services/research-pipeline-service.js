...
const pool = new Pool({
  connectionString:
    process.env.DATABASE_URL || 'postgresql://postgres:postgres@localhost:5432/dom_space_harvester',
  max: 20,
...
class ResearchPipelineService {
  ...
  async loadArticleIndex() {
    try {
      const indexPath = path.join(
        __dirname,
        '../docs/research/deepseek-ocr-contexts-optical-compression/AI_SERIES_INDEX.md'
      );
  ...
  async startMining(options = {}) {
    ...
    const { priority = 'high', limit = 50, skipExtracted = true } = options;
  ...
  async createResearchSeed(articleId, source) {
    ...
    try {
      await this.db.query(query, [source.url, source.type, articleId, JSON.stringify(source)]);
...
Source: ${content.source_url}
          ...
          messages: [
            {
              role: 'system',
              content:
                'You are an AI research analyst helping to understand and suggest connections between research articles for a DOM optimization platform called LightDom.',
            },
            {
              role: 'user',
  ...
  async processDeepSeekQueue() {
    ...
    for (const item of items) {
      ...
      try {
        // Mark as processing
        await this.db.query('UPDATE content_queue SET status = $1, sent_at = NOW() WHERE id = $2', [
          'sent-to-deepseek',
          item.id,
        ]);
        ...
        if (response) {
          ...
          await this.db.query('UPDATE content_queue SET status = $1 WHERE id = $2', [
            'failed',
            item.id,
          ]);
