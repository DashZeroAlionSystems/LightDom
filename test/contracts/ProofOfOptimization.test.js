...
describe('ProofOfOptimization', function () {
  ...
  describe('PoO Submission', function () {
    it('Should allow valid PoO submission', async function () {
      ...
      await expect(
        pooContract
          .connect(submitter)
          .submitPoO(crawlId, merkleRoot, bytesSaved, backlinksCount, artifactCID)
      )
        ...
        .withArgs(
          ...
          backlinksCount,
          artifactCID,
          await pooContract.getChallengeWindowEnds(crawlId)
        );
    });
  ...
  describe('PoO Challenge', function () {
    ...
    it('Should allow PoO challenge within window', async function () {
      ...
      await expect(pooContract.connect(challenger).challengePoO(crawlId, merkleProof, leafData))
  ...
  describe('PoO Finalization', function () {
    ...
    it('Should allow owner to finalize PoO', async function () {
      ...
      await expect(pooContract.finalizePoO(crawlId))
    ...
    it('Should reject finalization by non-owner', async function () {
      ...
      await expect(pooContract.connect(submitter).finalizePoO(crawlId)).to.be.revertedWith(
  ...
  describe('PoO Slashing', function () {
    ...
    it('Should allow owner to slash invalid PoO', async function () {
      ...
      await expect(pooContract.slashPoO(crawlId))
    ...
    it('Should reject slashing by non-owner', async function () {
      ...
      await expect(pooContract.connect(submitter).slashPoO(crawlId)).to.be.revertedWith(
        'Ownable: caller is not the owner'
      );
    });
  });
  ...
  describe('Batch Operations', function () {
    it('Should allow batch PoO submission', async function () {
      ...
      await expect(pooContract.connect(submitter).submitBatchPoO(batch, batchHash, signature))
