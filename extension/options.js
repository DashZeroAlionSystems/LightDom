...
class LightDomOptions {
  ...
  async loadStatistics() {
    try {
      ...
      document.getElementById('totalSpaceSaved').textContent = this.formatBytes(
        metrics.spaceSaved || 0
      );
  ...
  async loadRulesList() {
    try {
      ...
      if (response.success) {
        ...
        if (rules.length === 0) {
          rulesList.innerHTML =
            '<div style="text-align: center; opacity: 0.6; padding: 20px;">No rules configured</div>';
          return;
        }
  ...
  showAddRuleDialog() {
    ...
    const resourceTypes = prompt(
      'Enter resource types (comma-separated, e.g., script,stylesheet):',
      'script,stylesheet'
    );
    ...
    this.addCustomRule(
      urlPattern,
      resourceTypes.split(',').map(t => t.trim())
    );
  }
