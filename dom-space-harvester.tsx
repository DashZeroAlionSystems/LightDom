...
const RealWebCrawlerDashboard = () => {
  ...
  useEffect(() => {
    const generateCrawlers = () => {
      const crawlers = Array.from({ length: 6 }, (_, i) => ({
        ...
        currentUrl:
          Math.random() > 0.3
            ? realWebsites[Math.floor(Math.random() * realWebsites.length)].url
            : null,
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        ...
        const [parcels, biomesData] = await Promise.all([parcelsRes.json(), biomesRes.json()]);
  ...
  const fetchOnChain = async (url: string) => {
    try {
      ...
      alert('Failed to load on-chain record');
    }
  };
    ...
    <>
        ...
        <div className='max-w-7xl mx-auto mb-8'>
          <div className='text-center mb-8'>
            ...
            <p className='text-slate-300 text-lg'>
              Production-Scale Web Mining with PostgreSQL & Schema.org Integration
            </p>
          ...
          <div className='flex justify-center gap-4 mb-8'>
            <button
              ...
              className={`px-6 py-3 rounded-lg font-semibold transition-all duration-200 flex items-center gap-2 ${
                isCrawling ? 'bg-red-600 hover:bg-red-700' : 'bg-green-600 hover:bg-green-700'
              }`}
            >
            ...
            <div className='bg-slate-800/50 backdrop-blur rounded-xl p-4 border border-slate-700'>
              ...
              <button
                onClick={async () => {
                  ...
                  const token =
                    adminToken ||
                    prompt('Enter admin bearer token (required for demo route)') ||
                    '';
              ...
              {apiKey && <div className='mt-2 text-xs text-green-400 break-all'>Key: {apiKey}</div>}
              {apiKey && (
                <button
                  onClick={refreshUsage}
                  className='mt-2 px-3 py-1 bg-slate-700 hover:bg-slate-600 rounded text-xs'
                >
                  Refresh Usage
                </button>
              )}
              {usage && (
                <div className='mt-2 text-xs text-slate-400'>
                  Plan: <span className='text-blue-300'>{usage.plan}</span> • Used{' '}
                  {usage.requestsUsed}/{usage.requestsIncluded}
                </div>
              )}
            </div>
            ...
            <div className='bg-slate-800/50 backdrop-blur rounded-xl p-4 border border-slate-700'>
              ...
              <div className='flex gap-2'>
                ...
                <input
                  ...
                  onChange={e =>
                    setNewBounty({ ...newBounty, reward: parseInt(e.target.value, 10) })
                  }
                />
              </div>
              <button
                onClick={async () => {
                  ...
                  await fetch('/api/bounties', {
                    ...
                    body: JSON.stringify({ url: newBounty.url, rewardCents: newBounty.reward }),
                  });
                  alert('Bounty posted');
                }}
            ...
            <div className='bg-slate-800/50 backdrop-blur rounded-xl p-4 border border-slate-700'>
              ...
              <button
                onClick={async () => {
                  ...
                  if (!url) return;
                  if (!apiKey) return alert('Create an API key first');
                  const res = await fetch('/api/reports/optimization', {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json', 'x-api-key': apiKey },
          ...
          <div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-6 mb-8'>
            <div className='bg-slate-800/50 backdrop-blur rounded-xl p-6 border border-slate-700'>
              ...
              <div className='mt-2 text-xs text-slate-500'>
                ...
                {blockchainStats.contractAddress
                  ? `${blockchainStats.contractAddress.slice(0, 6)}...${blockchainStats.contractAddress.slice(-4)}`
                  : 'Not connected'}
              </div>
            </div>
            ...
            <div className='bg-slate-800/50 backdrop-blur rounded-xl p-6 border border-slate-700'>
              ...
              <div className='mt-2 text-xs text-slate-500'>From PoO submissions</div>
            </div>
            ...
            <div className='bg-slate-800/50 backdrop-blur rounded-xl p-6 border border-slate-700'>
              ...
              <div className='mt-2 text-xs text-slate-500'>Real-time updates</div>
            </div>
            ...
            <div className='bg-slate-800/50 backdrop-blur rounded-xl p-6 border border-slate-700'>
              ...
              <div className='mt-2 text-xs text-slate-500'>Infrastructure building</div>
            </div>
          </div>
          ...
          <div className='grid grid-cols-1 xl:grid-cols-3 gap-8'>
            ...
            <div className='bg-slate-800/50 backdrop-blur rounded-xl p-6 border border-slate-700'>
              <h3 className='text-xl font-bold mb-4 flex items-center gap-2'>
                ...
                Distributed Crawlers ({activeCrawlers.filter(c => c.status === 'active').length}/6
                active)
              </h3>
              <div className='space-y-3'>
                {activeCrawlers.map(crawler => (
                  <div
                    key={crawler.id}
                    className='bg-gradient-to-r from-slate-700/50 to-slate-600/30 rounded-lg p-3 border-l-4 border-blue-500'
                  >
                    <div className='flex justify-between items-center mb-2'>
                      <span className='font-semibold'>Crawler #{crawler.id}</span>
                      <div className='flex items-center gap-2'>
                        <div
                          className={`w-2 h-2 rounded-full ${crawler.status === 'active' ? 'bg-green-400' : 'bg-slate-500'}`}
                        ></div>
              ...
              <div className='space-y-3'>
                {recentOptimizations.length === 0 ? (
                  ...
                  recentOptimizations.map(opt => (
                    <div
                      key={opt.id}
                      className='bg-gradient-to-r from-slate-700/50 to-slate-600/30 rounded-lg p-3'
                    >
                      <div className='flex justify-between items-start mb-2'>
                        ...
                        <div
                          className={`text-xs px-2 py-1 rounded bg-slate-600 ${getAuthorityColor(opt.authority)}`}
                        >
                      ...
                      <div className='grid grid-cols-2 gap-2 text-xs mb-2'>
                        ...
                        <span className='text-orange-400'>
                          🌉 +{opt.metaverseImpact.bridgeLinks} bridges
                        </span>
                      </div>
            ...
            <div className='bg-slate-800/50 backdrop-blur rounded-xl p-6 border border-slate-700'>
              ...
              <div className='space-y-4'>
                <div className='flex items-center gap-3'>
                  <canvas
                    ref={biodomeCanvasRef}
                    className='rounded border border-slate-600'
                    style={{ imageRendering: 'pixelated' as any }}
                  />
                  <div>
                    <button
                      onClick={() => setBiomeSeed(Math.floor(Math.random() * 1e6))}
              ...
              <div className='space-y-3 max-h-96 overflow-auto'>
                {liveOptimizations.length === 0 ? (
                  ...
                  liveOptimizations.map(opt => (
                    <div
                      key={opt.id}
                      className='bg-gradient-to-r from-slate-700/50 to-slate-600/30 rounded-lg p-3 border-l-4 border-emerald-500'
                    >
                      <div className='flex justify-between items-start mb-2'>
                        <div className='flex items-center gap-2'>
                      ...
                      <div className='text-sm text-slate-300 mb-2'>
                        {opt.url && (
                          <div className='font-mono text-xs text-slate-400 mb-1'>{opt.url}</div>
                        )}
                        <div className='flex gap-4'>
                          <span>
                            Bytes:{' '}
                            <span className='text-emerald-400 font-mono'>
                              {opt.bytesSaved?.toLocaleString() || 'N/A'}
                            </span>
                          </span>
                          <span>
                            Backlinks:{' '}
                            <span className='text-blue-400 font-mono'>
                              {opt.backlinksCount || 0}
                            </span>
                          </span>
                        </div>
                      </div>
              ...
              <div className='space-y-3 max-h-96 overflow-auto'>
                {metaverseEvents.length === 0 ? (
                  ...
                  metaverseEvents.map(event => (
                    <div
                      key={event.id}
                      className='bg-gradient-to-r from-slate-700/50 to-slate-600/30 rounded-lg p-3 border-l-4 border-violet-500'
                    >
                      <div className='flex justify-between items-start mb-2'>
                        <div className='flex items-center gap-2'>
          ...
          <div className='mt-8 grid grid-cols-1 md:grid-cols-2 gap-8'>
            ...
            <div className='bg-gradient-to-r from-slate-800/50 via-blue-800/20 to-slate-800/50 backdrop-blur rounded-xl p-6 border border-blue-500/30'>
              ...
              <div className='space-y-3'>
                {plans.map(p => (
                  <div
                    key={p.plan_code}
                    className='p-3 rounded bg-slate-700/40 flex justify-between'
                  >
                    <div>
                      <div className='font-semibold'>{p.name}</div>
                      <div className='text-xs text-slate-400'>
                        Includes {p.requests_included} requests • $
                        {(p.monthly_price_cents / 100).toFixed(2)}/mo
                      </div>
                    </div>
                    <div className='text-xs text-slate-400'>
                      Overage ${(p.overage_price_cents_per_1k / 100).toFixed(2)} / 1k
                    </div>
                  </div>
                ))}
              </div>
            ...
            <div className='bg-gradient-to-r from-slate-800/50 via-green-800/20 to-slate-800/50 backdrop-blur rounded-xl p-6 border border-green-500/30'>
              ...
              <div className='space-y-2 max-h-72 overflow-auto'>
                {recentProofs.map(p => (
                  <div
                    key={`${p.url}-${p.recorded_at}`}
                    className='p-3 rounded bg-slate-700/40 text-xs'
                  >
                    <div className='flex justify-between'>
                      <span className='text-blue-300 break-all'>{p.url}</span>
                      <span className={`ml-2 ${p.on_chain ? 'text-green-400' : 'text-yellow-400'}`}>
                        {p.on_chain ? 'on-chain' : 'queued'}
                      </span>
                    </div>
                    <div className='text-slate-400 mt-1'>
                      Saved: {(p.space_saved_bytes / 1024).toFixed(1)} KB
                    </div>
                    {p.tx_hash && (
                  ...
                  {domainProofs.map(p => (
                    ...
                    >
                      <div className='flex justify-between'>
                        <span className='text-blue-300 break-all'>{p.url}</span>
                        <span
                          className={`ml-2 ${p.on_chain ? 'text-green-400' : 'text-yellow-400'}`}
                        >
                          {p.on_chain ? 'on-chain' : 'queued'}
                        </span>
                      </div>
            ...
            <div className='bg-gradient-to-r from-slate-800/50 via-blue-800/20 to-slate-800/50 backdrop-blur rounded-xl p-6 border border-blue-500/30'>
              ...
              <div className='grid grid-cols-2 gap-4'>
                <div className='text-center'>
                  <div className='text-2xl font-bold text-blue-400'>
                    {Object.keys(schemaStats.schemaTypes).length}
                  </div>
                ...
                <div className='text-center'>
                  <div className='text-2xl font-bold text-yellow-400'>
                    {(backlinkNetwork.avgStrength * 100).toFixed(0)}%
                  </div>
              ...
              <div className='mt-4'>
                <h4 className='text-sm font-semibold text-slate-300 mb-2'>
                  Top Schema Types Found:
                </h4>
                <div className='space-y-1'>
                  {Object.entries(schemaStats.schemaTypes)
                    .slice(0, 5)
                    .map(([type, count]) => (
                ...
                <div className='flex justify-between items-center p-3 bg-slate-700/30 rounded'>
                  ...
                  <span className='text-green-400 font-bold'>
                    {crawlerStats.totalTargets > 0
                      ? ((crawlerStats.completed / crawlerStats.totalTargets) * 100).toFixed(1)
                      : 0}
                    %
                  </span>
                </div>
                ...
                <div className='flex justify-between items-center p-3 bg-slate-700/30 rounded'>
                  ...
                  <span className='text-blue-400 font-bold'>
                    {crawlerStats.sitesAnalyzed > 0
                      ? (crawlerStats.totalSpaceHarvested / crawlerStats.sitesAnalyzed).toFixed(1)
                      : 0}{' '}
                    KB
                  </span>
                </div>
          ...
          <div className='mt-8 bg-slate-800/50 backdrop-blur rounded-xl p-6 border border-slate-700'>
            <h3 className='text-xl font-bold mb-4'>
              How Real Web Crawling & DOM Space Harvesting Works
            </h3>
            <div className='grid grid-cols-1 md:grid-cols-4 gap-6'>
              <div className='text-center'>
                ...
                <p className='text-slate-400 text-sm'>
                  Crawlers discover real websites via sitemaps, robots.txt, and link analysis
                </p>
              </div>
              <div className='text-center'>
                ...
                <p className='text-slate-400 text-sm'>
                  All crawl data, schemas, and optimizations stored in production database
                </p>
              </div>
              <div className='text-center'>
                ...
                <p className='text-slate-400 text-sm'>
                  Extract schema.org data and map backlink networks for real websites
                </p>
              </div>
              <div className='text-center'>
                ...
                <p className='text-slate-400 text-sm'>
                  Convert real web optimizations into blockchain infrastructure and NFT assets
                </p>
              </div>
            </div>
          </div>
        ...
        <div className='fixed bottom-4 right-4 w-80 bg-slate-900/90 border border-slate-700 rounded-xl shadow-lg backdrop-blur'>
          ...
          <div className='p-3 max-h-64 overflow-auto text-xs space-y-2'>
            {!chatBridge &&
              bridges.map((b: any) => (
                <div
                  key={b.bridge_id}
                  className='w-full p-2 rounded bg-slate-800/60 hover:bg-slate-700'
                >
                  <div className='flex justify-between items-start mb-2'>
                    <div className='flex-1'>
                      <div className='flex justify-between'>
                        <span className='text-blue-300'>
                          {b.source_chain} → {b.target_chain}
                        </span>
                        <span className={b.is_operational ? 'text-green-400' : 'text-red-400'}>
                          {b.is_operational ? 'online' : 'offline'}
                        </span>
                      </div>
                      <div className='text-slate-400 text-xs'>
                        Capacity: {Number(b.bridge_capacity || 0).toLocaleString()}
                      </div>
                      <div className='text-slate-400 text-xs'>
                        Volume: {Number(b.current_volume || 0).toLocaleString()}
                      </div>
              ...
              <div className='space-y-2'>
                <div className='flex justify-between items-center'>
                  ...
                  <button
                    ...
                    onClick={() => {
                      if (socketRef.current) socketRef.current.emit('bridge_leave', chatBridge);
                      setChatBridge(null);
                      setChatMessages([]);
                    }}
                  >
                ...
                <div className='max-h-40 overflow-auto space-y-1'>
                  {chatMessages.map((m, i) => (
                    <div key={i} className='p-1 rounded bg-slate-800/60'>
                      <span className='text-blue-300'>{m.user}:</span>{' '}
          ...
          <div className='p-2 border-t border-slate-700 flex gap-2'>
            <input
              ...
              placeholder={chatBridge ? 'Message…' : 'Select a bridge to join…'}
              disabled={!chatBridge}
            />
            <button
              onClick={sendChat}
      ...
      {!showChat && (
        <button
          ...
          className='fixed bottom-4 right-4 px-3 py-2 bg-slate-900/90 border border-slate-700 rounded-xl text-xs'
        >
          Open Metaverse
        </button>
      )}
