...
class N8nMCPCLI {
    ...
    async listWorkflows() {
        ...
        try {
            ...
            workflows.forEach((workflow) => {
                const status = workflow.active ? chalk_1.default.green('● Active') : chalk_1.default.gray('○ Inactive');
    ...
    async createSampleWorkflow() {
        ...
        try {
            const response = await axios_1.default.post(`${this.config.baseUrl}/api/v1/workflows`, sampleWorkflow, {
                headers: {
    ...
    async exportWorkflow(workflowId, outputPath) {
        ...
        try {
            const response = await axios_1.default.get(`${this.config.baseUrl}/api/v1/workflows/${workflowId}`, {
                headers: this.config.apiKey ? { 'X-N8N-API-KEY': this.config.apiKey } : {},
                timeout: this.config.timeout,
            });
            const exportData = {
    ...
    async importWorkflow(filePath, newName) {
        ...
        try {
            ...
            const response = await axios_1.default.post(`${this.config.baseUrl}/api/v1/workflows`, workflowData, {
                headers: {
                    'Content-Type': 'application/json',
                    ...(this.config.apiKey ? { 'X-N8N-API-KEY': this.config.apiKey } : {}),
                },
    ...
    async generateConfig() {
        ...
        try {
            ...
            console.error(chalk_1.default.red(`Failed to save config: ${error instanceof Error ? error.message : String(error)}`));
        }
    }
    async startServer() {
