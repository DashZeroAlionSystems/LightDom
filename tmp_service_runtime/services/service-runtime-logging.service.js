...
export class ServiceRuntimeLoggingService {
     ...
     */
    async createRuntimeProfile(input) {
        return this.runInTransaction(async (client) => {
            if (input.isDefault) {
                await client.query('UPDATE service_runtime_profiles SET is_default = false WHERE service_id = $1', [input.serviceId]);
            }
            const result = await client.query(`INSERT INTO service_runtime_profiles (
     ...
     */
    async updateRuntimeProfile(profileId, patch) {
        return this.runInTransaction(async (client) => {
            if (patch.isDefault === true) {
                const { rows } = await client.query('SELECT service_id FROM service_runtime_profiles WHERE profile_id = $1', [profileId]);
                if (rows[0]) {
                    await client.query('UPDATE service_runtime_profiles SET is_default = false WHERE service_id = $1', [rows[0].service_id]);
                }
            }
            const fields = [];
            const values = [];
            ...
            if (fields.length === 0) {
                const { rows } = await client.query('SELECT * FROM service_runtime_profiles WHERE profile_id = $1', [profileId]);
                return rows[0] ? this.mapProfile(rows[0]) : null;
            }
            values.push(profileId);
            const result = await client.query(`UPDATE service_runtime_profiles
     ...
     */
    async getRuntimeProfile(profileId) {
        const result = await this.db.query('SELECT * FROM service_runtime_profiles WHERE profile_id = $1', [profileId]);
        return result.rows[0] ? this.mapProfile(result.rows[0]) : null;
    }
    /**
     * List runtime profiles with optional filtering.
     */
    async listRuntimeProfiles(filters = {}) {
        const conditions = [];
        const values = [];
        ...
        const result = await this.db.query(`SELECT * FROM service_runtime_profiles ${whereClause}
     ...
     */
    async createStreamChannel(input) {
        const result = await this.db.query(`INSERT INTO service_stream_channels (
     ...
     */
    async updateStreamChannel(channelId, patch) {
        const fields = [];
        const values = [];
        if (patch.name !== undefined) {
            fields.push(`name = $${fields.length + 1}`);
        ...
        if (fields.length === 0) {
            const result = await this.db.query('SELECT * FROM service_stream_channels WHERE channel_id = $1', [channelId]);
            return result.rows[0] ? this.mapChannel(result.rows[0]) : null;
        }
        values.push(channelId);
        const result = await this.db.query(`UPDATE service_stream_channels
       SET ${fields.join(', ')}, updated_at = CURRENT_TIMESTAMP
     ...
     */
    async listStreamChannels(serviceId) {
        const result = await this.db.query(`SELECT * FROM service_stream_channels
       WHERE service_id = $1
    ...
    async createServiceSession(input) {
        const result = await this.db.query(`INSERT INTO service_sessions (
     ...
     */
    async updateServiceSession(sessionId, patch) {
        const fields = [];
        const values = [];
        ...
        if (fields.length === 0) {
            const result = await this.db.query(`SELECT ss.*, srp.name AS profile_name, srp.profile_key, srp.environment AS profile_environment
        ...
        const result = await this.db.query(`UPDATE service_sessions
       SET ${fields.join(', ')}, updated_at = CURRENT_TIMESTAMP
       ...
       RETURNING *`, values);
        ...
        const enriched = await this.db.query(`SELECT ss.*, srp.name AS profile_name, srp.profile_key, srp.environment AS profile_environment
       FROM service_sessions ss
       LEFT JOIN service_runtime_profiles srp ON ss.profile_id = srp.profile_id
     ...
     */
    async listServiceSessions(filters = {}) {
        const conditions = [];
        const values = [];
        if (filters.serviceId) {
     ...
     */
    async getServiceSessionDetails(sessionId, options = {}) {
        const base = await this.db.query(`SELECT ss.*, srp.name AS profile_name, srp.profile_key, srp.environment AS profile_environment
       FROM service_sessions ss
       LEFT JOIN service_runtime_profiles srp ON ss.profile_id = srp.profile_id
       WHERE ss.service_session_id = $1`, [sessionId]);
        if (!base.rows[0]) {
        ...
        const detail = this.mapSession(base.rows[0]);
        if (options.includeEvents) {
            const eventResult = await this.db.query(`SELECT * FROM service_session_events
        ...
        if (options.includeStreams) {
            const streamResult = await this.db.query(`SELECT * FROM service_session_streams
         WHERE service_session_id = $1
        ...
        if (options.includeMetrics) {
            const metricResult = await this.db.query(`SELECT * FROM service_session_metrics
         WHERE service_session_id = $1
     ...
     */
    async logServiceSessionEvent(input) {
        const result = await this.db.query(`INSERT INTO service_session_events (
     ...
     */
    async recordServiceSessionStream(input) {
        const result = await this.db.query(`WITH next_sequence AS (
     ...
     */
    async recordServiceSessionMetric(input) {
        const result = await this.db.query(`INSERT INTO service_session_metrics (
