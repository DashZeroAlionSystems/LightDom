...
export class MVPFeatureCollector {
  ...
  async collectMVPFeatures(url: string, keyword: string): Promise<MVPFeatures> {
    ...
    const derivedFeatures = this.calculateDerivedFeatures(rawFeatures);
   ...
   */
  private async collectWebScrapingFeatures(url: string, keyword: string): Promise<Partial<MVPFeatures>> {
    try {
    ...
    try {
      ...
      const features: Partial<MVPFeatures> = {
        ...
        inp_ms: audits['interaction-to-next-paint']?.numericValue || 
                audits['max-potential-fid']?.numericValue || 500, // Fallback to FID if INP not available
        cls_score: audits['cumulative-layout-shift']?.numericValue || 0.25
      };

      console.log(`✓ PageSpeed API complete: LCP=${features.lcp_ms}ms, INP=${features.inp_ms}ms, CLS=${features.cls_score}`);
      return features;
   ...
   */
  private calculateDerivedFeatures(rawFeatures: Partial<MVPFeatures>): Partial<MVPFeatures> {
    const derivedFeatures: Partial<MVPFeatures> = {};

    // 1. Title optimal (50-60 characters)
    derivedFeatures.title_optimal = 
      rawFeatures.title_length! >= 50 && 
      rawFeatures.title_length! <= 60;
    ...
    derivedFeatures.overall_seo_score = (
      derivedFeatures.content_quality * 0.30 +       // 30% content
      derivedFeatures.technical_health * 0.30 +      // 30% technical
      (rawFeatures.ctr! / getExpectedCTR(rawFeatures.position!)) * 0.20 + // 20% engagement
      (derivedFeatures.title_optimal ? 0.20 : 0)     // 20% on-page
    );

    console.log(`✓ Derived features calculated: Overall SEO Score = ${(derivedFeatures.overall_seo_score * 100).toFixed(1)}%`);
  ...
  analyzeResults(features: MVPFeatures): {
    ...
    if (features.meta_desc_length < 150 || features.meta_desc_length > 160) {
      quickWins.push(`Optimize meta description to 150-160 characters (current: ${features.meta_desc_length})`);
    }
    ...
    if (ctrRatio < 0.8) {
      issues.push(`CTR underperforming: ${(features.ctr * 100).toFixed(1)}% vs ${(expectedCtr * 100).toFixed(1)}% expected`);
    }
...
export async function runMVPAnalysis(url: string, keyword: string) {
  ...
  console.log('\n🌐 Web Scraping (FREE):');
  console.log(`  Title Length: ${features.title_length} chars ${features.title_optimal ? '✅' : '❌'}`);
