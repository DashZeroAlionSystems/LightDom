  ...
  return (
    <div className='min-h-screen bg-gray-50'>
        ...
        <div className='max-w-7xl mx-auto px-4 sm:px-6 lg:px-8'>
          <div className='flex justify-between items-center h-16'>
            <div className='flex items-center'>
              <h1 className='text-2xl font-bold text-gray-900'>LightDom Project Management</h1>
      ...
      <main className='max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8'>
        ...
        {state.view === 'projects' && (
          <ProjectsView state={state} onProjectSelect={handleProjectSelect} />
        )}
        {state.view === 'nodes' && <NodesView state={state} />}
        {state.view === 'workflows' && (
          <WorkflowsView state={state} onExecuteWorkflow={handleExecuteWorkflow} />
        )}
        ...
        <div className='bg-white rounded-lg shadow p-6'>
          ...
          <div className='space-y-3'>
            <MetricRow
              ...
              value={metrics?.blockchainMetrics?.transactionsProcessed || 0}
            />
            <MetricRow label='Blocks Mined' value={metrics?.blockchainMetrics?.blocksMined || 0} />
            <MetricRow
              label='Optimizations Submitted'
              value={metrics?.blockchainMetrics?.optimizationsSubmitted || 0}
            />
            <MetricRow label='Gas Used' value={metrics?.blockchainMetrics?.gasUsed || 0} />
            <MetricRow
              label='Error Rate'
              value={`${metrics?.blockchainMetrics?.errorRate || 0}%`}
            />
          </div>
        </div>
...
const MetricCard: React.FC<{
  ...
  return (
    <div className='bg-white rounded-lg shadow p-6'>
      <div className='flex items-center'>
        <div
          className={`p-3 rounded-full ${colorClasses[color as keyof typeof colorClasses]} text-white`}
        >
  ...
  return (
    ...
    >
      ...
      <div className='mt-4 flex justify-between text-sm text-gray-500'>
        <span>
          {project.metrics?.tasksCompleted || 0} / {project.metrics?.tasksTotal || 0} tasks
        </span>
        <span>
          ${project.metrics?.budgetUsed || 0} / ${project.budget || 0}
        </span>
      </div>
    </div>
  );
