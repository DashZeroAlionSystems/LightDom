...
const SpaceOptimizationDashboard: React.FC = () => {
  ...
  const simulateOptimization = () => {
    ...
    const optimization: OptimizationResult = {
      ...
      metaverseAssets:
        Math.floor(spaceFound / 100) + Math.floor(spaceFound / 500) + Math.floor(spaceFound / 1000),
    };
  ...
  const submitOptimization = async () => {
    ...
    try {
      ...
      if (data.success) {
        ...
        alert(
          `Optimization submitted! ${data.data.spaceSavedKB}KB saved, ${data.data.tokenReward.toFixed(6)} DSH earned`
        );
      } else {
      ...
      <div className='max-w-7xl mx-auto mb-8'>
        ...
        <div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8'>
          ...
          <div className='bg-slate-800/50 backdrop-blur rounded-xl p-6 border border-slate-700'>
            ...
            <div className='mt-2 text-xs text-slate-500'>From space optimization rewards</div>
          </div>
          ...
          <div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4'>
            ...
            <input
              ...
              onChange={e =>
                setNewOptimization({
                  ...newOptimization,
                  spaceSavedBytes: parseInt(e.target.value) || 0,
                })
              }
              className='bg-slate-900 border border-slate-700 rounded px-3 py-2 text-sm'
            />
            <select
              ...
              onChange={e =>
                setNewOptimization({ ...newOptimization, optimizationType: e.target.value })
              }
              className='bg-slate-900 border border-slate-700 rounded px-3 py-2 text-sm'
            >
            ...
            <div className='space-y-3 max-h-96 overflow-y-auto'>
              {harvesters.length === 0 ? (
                ...
                harvesters.map((harvester, index) => (
                  <div
                    key={harvester.address}
                    className='bg-gradient-to-r from-slate-700/50 to-slate-600/30 rounded-lg p-3'
                  >
                    <div className='flex justify-between items-center mb-2'>
                      <span className='font-semibold'>
                        #{index + 1} {formatAddress(harvester.address)}
                      </span>
                    ...
                    <div className='grid grid-cols-2 gap-2 text-xs mb-2'>
                      <span className='text-blue-400'>
                        💾 {Math.floor(harvester.totalSpaceHarvested / 1024)} KB
                      </span>
                      <span className='text-yellow-400'>
                        💰 {harvester.totalTokensEarned.toFixed(2)} DSH
                      </span>
                    ...
                    <div className='text-xs text-slate-400'>
                      {harvester.optimizationCount} optimizations •{' '}
                      {harvester.stakingRewards.toFixed(2)} DSH staking
                    </div>
                  </div>
                ))
              )}
            ...
            <div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6'>
              ...
              <div className='text-center'>
                <div className='text-2xl font-bold text-green-400'>
                  {optimizations.length > 0
                    ? Math.round(
                        optimizations.reduce((sum, opt) => sum + opt.qualityScore, 0) /
              ...
              <div className='text-center'>
                <div className='text-2xl font-bold text-purple-400'>
                  {optimizations.length > 0
                    ? Math.round(
                        optimizations.reduce((sum, opt) => sum + opt.spaceSavedKB, 0) /
                          optimizations.length
                      )
                    : 0}{' '}
                  KB
              ...
              <div className='text-center'>
                <div className='text-2xl font-bold text-yellow-400'>
                  {optimizations.length > 0
                    ? (
                        optimizations.reduce((sum, opt) => sum + opt.tokensEarned, 0) /
                        optimizations.length
                      ).toFixed(4)
                    : 0}{' '}
                  DSH
