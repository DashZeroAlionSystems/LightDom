...
const WalletDashboard: React.FC<WalletDashboardProps> = ({ className }) => {
      ...
      render: (amount: number, record: any) => (
        <div className="amount-cell">
          <Text strong style={{ color: record.type === 'received' ? '#52c41a' : '#f5222d' }}>
            {record.type === 'received' ? '+' : '-'}{amount} {record.token}
          </Text>
          <br />
  ...
  const handleSendTokens = async (values: any) => {
    try {
      await sendTokens({ recipient: values.recipient, amount: Number(values.amount), token: values.token });
        ...
        <Card className="connect-card">
          <div className="connect-content">
            ...
            <Paragraph type="secondary">
              Connect your wallet to view your tokens, transactions, and manage your LightDom assets.
            </Paragraph>
            <Button
              type="primary"
              size="large"
    ...
    <div className={`wallet-dashboard ${className || ''}`}>
      ...
      <Card className="dashboard-header" bordered={false}>
        <Row align="middle" justify="space-between">
          <Col>
            ...
            <Paragraph type="secondary">
              Manage your LightDom tokens and transactions
            </Paragraph>
          </Col>
          <Col>
            <Space>
                      ...
                      <List.Item.Meta
                        ...
                        description={
                          <div className="token-description">
                            <Text type="secondary">Balance: {token.balance} {token.symbol}</Text>
                            <br />
                            <Text type="secondary">Value: ${(token.balance * token.price).toFixed(2)}</Text>
                          </div>
                        }
                      />
      ...
      >
        <div className="receive-content">
          ...
          <Alert
            message="Share this address to receive tokens"
            type="info"
            showIcon
          />
        </div>
      ...
      >
        <Form form={claimForm} layout="vertical" onFinish={handleClaimRewards}>
          ...
          <Alert
            message={`Available: ${walletStats.pendingRewards} LDT`}
            type="info"
            showIcon
          />
        </Form>
