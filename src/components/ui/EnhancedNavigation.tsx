...
import { 
  ...
  Rocket, BarChart3, Bug, Server, ShieldCheck, Search, Trophy
} from 'lucide-react';
...
const EnhancedNavigation: React.FC<NavigationProps> = ({ currentPath = window.location.pathname, onNavigate, children }) => {
  ...
  const items = useMemo(() => {
    const all = [
      ...
      { path: '/metaverse-mining', label: 'Metaverse Mining', icon: Globe, plans: ['pro','enterprise','admin'] },
      { path: '/metaverse-marketplace', label: 'Metaverse Marketplace', icon: Globe, plans: ['pro','enterprise','admin'] },
      { path: '/metaverse-mining-rewards', label: 'Mining Rewards', icon: Trophy, plans: ['pro','enterprise','admin'] },
      { path: '/space-optimization', label: 'Space Optimization', icon: BarChart3, plans: ['pro','enterprise','admin'] },
      { path: '/seo-optimization', label: 'SEO Optimization', icon: Search, plans: ['pro','enterprise','admin'] },
      { path: '/seo-datamining', label: 'SEO Data Mining', icon: Search, plans: ['pro','enterprise','admin'] },
      { path: '/seo-marketplace', label: 'SEO Marketplace', icon: Search, plans: ['pro','enterprise','admin'] },
      { path: '/workflow-simulation', label: 'Workflow Simulation', icon: Workflow, plans: ['enterprise','admin'] },
      { path: '/testing', label: 'Testing Dashboard', icon: Bug, plans: ['enterprise','admin'] },
      ...
      { path: '/blockchain', label: 'Blockchain Storage', icon: Database, plans: ['pro','enterprise','admin'] },
      { path: '/websites', label: 'Websites', icon: Globe, plans: ['free','pro','enterprise','admin'] },
      { path: '/history', label: 'History', icon: BarChart3, plans: ['free','pro','enterprise','admin'] },
      { path: '/achievements', label: 'Achievements', icon: Trophy, plans: ['free','pro','enterprise','admin'] },
      { path: '/marketplace', label: 'Marketplace', icon: Globe, plans: ['pro','enterprise','admin'] },
      { path: '/workflow-demo', label: 'Workflow Demo', icon: Workflow, plans: ['admin'] },
  ...
  return (
    <div style={{ display: 'flex', height: '100vh', width: '100vw' }}>
      {/* Navigation Sidebar */}
      <aside style={{ position: 'relative', width: expanded ? NAV_WIDTH_RAIL + NAV_WIDTH_PANEL : NAV_WIDTH_RAIL, flexShrink: 0, zIndex: 120 }}>
        {/* Rail */}
        <div style={{ width: NAV_WIDTH_RAIL, background: '#0f172a', color: 'rgba(255,255,255,0.9)', display: 'flex', flexDirection: 'column', justifyContent: 'space-between', alignItems: 'center', padding: '12px 8px', boxShadow: '2px 0 8px rgba(0,0,0,.3)', height: '100vh', position: 'fixed', top: 0, left: 0 }}>
        ...
        {/* Panel */}
        {expanded && (
          <div style={{ width: NAV_WIDTH_PANEL, background: '#0b1220', color: '#e6e8ef', display: 'flex', flexDirection: 'column', borderLeft: '1px solid #0a1a30', height: '100vh', position: 'fixed', top: 0, left: NAV_WIDTH_RAIL }}>
            ...
            <nav style={{ padding: '8px 8px 0 8px', overflowY: 'auto', flex: 1 }}>
              {items.map(it => {
                const Icon = it.icon as any;
                const active = currentPath === it.path;
              return (
      ...
      <main style={{ 
        flex: 1, 
        marginLeft: expanded ? NAV_WIDTH_RAIL + NAV_WIDTH_PANEL : NAV_WIDTH_RAIL,
