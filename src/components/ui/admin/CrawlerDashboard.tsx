...
export const CrawlerDashboard: React.FC = () => {
  ...
  const crawlersColumns = [
    ...
    },
    {
      ...
      render: (efficiency: number) => (
        <Progress percent={efficiency} size="small" status={efficiency > 80 ? 'success' : 'normal'} />
      )
    },
    {
  ...
  const chartData = optimizations.slice(0, 20).reverse().map((opt, index) => ({
    index: index + 1,
  ...
  return (
    <div className="crawler-dashboard">
        ...
        <Col xs={24} sm={12} lg={6}>
          <Card>
            <Statistic
              ...
              value={activeCrawlers.filter(c => c.status === 'active' || c.status === 'processing').length}
      ...
      {stats?.statusBreakdown && stats.statusBreakdown.length > 0 && (
        <Card title="Crawl Status Breakdown" style={{ marginBottom: 24 }}>
          <Row gutter={16}>
              ...
              <Col key={status} xs={12} sm={8} md={6}>
                <Statistic
                  ...
                  prefix={
                    ...
                    status === 'processing' ? <SyncOutlined spin style={{ color: '#1890ff' }} /> :
      ...
      <Row gutter={[16, 16]} style={{ marginBottom: 24 }}>
        <Col xs={24} lg={12}>
          <Card title="Space Saved Trend">
            <ResponsiveContainer width="100%" height={300}>
              <AreaChart data={chartData}>
                <CartesianGrid strokeDasharray="3 3" />
                <XAxis dataKey="index" label={{ value: 'Recent Crawls', position: 'insideBottom', offset: -5 }} />
                <YAxis label={{ value: 'KB Saved', angle: -90, position: 'insideLeft' }} />
                <RechartsTooltip />
                <Legend />
                <Area type="monotone" dataKey="spaceSaved" stroke="#1890ff" fill="#1890ff" fillOpacity={0.6} name="Space Saved (KB)" />
              </AreaChart>
            </ResponsiveContainer>
          </Card>
        </Col>
        <Col xs={24} lg={12}>
          <Card title="Tokens Earned Trend">
            <ResponsiveContainer width="100%" height={300}>
              <LineChart data={chartData}>
                ...
                <RechartsTooltip />
                <Legend />
                <Line type="monotone" dataKey="tokensEarned" stroke="#faad14" strokeWidth={2} name="Tokens Earned" />
              </LineChart>
            </ResponsiveContainer>
          </Card>
        </Col>
