...
export const AgentSessionsPanel: React.FC<AgentSessionsPanelProps> = ({
    ...
    const menu = (
      <Menu>
        ...
        <Menu.Item
          ...
          icon={<DeleteOutlined />}
          danger
          onClick={() => handleDeleteAgent(agent.instance_id)}
        >
          Delete Agent
        </Menu.Item>
      </Menu>
        ...
        header={
          ...
          >
            <Space>
              ...
              <Tag
                color={session.agent_type === 'deepseek' ? 'blue' : 'default'}
                style={{ fontSize: 11 }}
              >
                {session.agent_type}
              </Tag>
  ...
  return (
    <div className='agent-sessions-panel'>
      <Card className='panel-header' size='small' bodyStyle={{ padding: '12px' }}>
      ...
      <div
        ...
        style={{ padding: '12px', maxHeight: 'calc(100vh - 200px)', overflowY: 'auto' }}
      >
        {loading ? (
      ...
      >
        <Form form={promptForm} layout='vertical' onFinish={handleStartAgentSession}>
          <Form.Item name='instructions' label='Instructions for Agent'>
            <TextArea rows={6} placeholder='Describe what you want the agent to work on...' />
          </Form.Item>
