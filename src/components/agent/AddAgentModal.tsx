...
export const AddAgentModal: React.FC<AddAgentModalProps> = ({ visible, onCancel, onSuccess }) => {
    ...
    try {
      ...
      const agentData = {
        ...
        deepseek_config: {
          ...
          model: values.model_name || 'deepseek-reasoner',
        },
        tools_enabled: values.tools_enabled || [],
        services_enabled: values.services_enabled || [],
        model_name: values.model_name || 'deepseek-reasoner',
      ...
      case 0:
        return (
          <div>
            ...
            >
              <Select placeholder='Select agent mode' onChange={handleModeSelect} size='large'>
                {agentModes.map(mode => (
        ...
        return (
          <div>
            ...
            <Form.Item name='description' label='Description'>
              <TextArea rows={3} placeholder='Describe what this agent will do...' />
            </Form.Item>

            <Form.Item name='tools_enabled' label='Enabled Tools'>
              <Select mode='tags' placeholder='Select or add tools' tokenSeparators={[',']}>
            ...
            <Form.Item name='services_enabled' label='Enabled Services'>
              <Select mode='tags' placeholder='Select or add services' tokenSeparators={[',']}>
      ...
      case 2:
        return (
          <div>
            ...
            <Form.Item name='model_name' label='Model Name' initialValue='deepseek-reasoner'>
              <Select>
            ...
            >
              <InputNumber min={100} max={16000} step={100} style={{ width: '100%' }} />
            </Form.Item>
        ...
        return (
          <div>
            ...
            <Card title='Agent Details' size='small' style={{ marginBottom: 16 }}>
              <p>
                <Text strong>Name:</Text> {values.name}
              </p>
              <p>
                <Text strong>Description:</Text> {values.description || 'None'}
              </p>
              <p>
                <Text strong>Mode:</Text> {selectedMode?.name}
              </p>
            </Card>

            <Card title='Configuration' size='small' style={{ marginBottom: 16 }}>
              <p>
                <Text strong>Tools:</Text> {values.tools_enabled?.join(', ') || 'None'}
              </p>
              <p>
                <Text strong>Services:</Text> {values.services_enabled?.join(', ') || 'None'}
              </p>
            </Card>

            <Card title='DeepSeek Settings' size='small'>
              <p>
                <Text strong>API URL:</Text> {values.deepseek_api_url}
              </p>
              <p>
                <Text strong>Model:</Text> {values.model_name}
              </p>
              <p>
                <Text strong>Temperature:</Text> {values.temperature}
              </p>
              <p>
                <Text strong>Max Tokens:</Text> {values.max_tokens}
              </p>
            </Card>
          </div>
        );
