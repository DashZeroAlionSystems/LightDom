import { AccordionItem, BotReplyBox, BusyIndicator } from '@/components/atoms/rag';
  ...
  render: () => {
    const [messages] = useState([
      {
        ...
        content:
          'I understand you want to create a new API endpoint for user authentication. Let me help you with that.',
        location: {
          repo: 'myproject/api',
      ...
      },
      {
        ...
        content:
          'Note: I noticed that the database schema needs to be updated to support refresh tokens. Would you like me to handle that as well?',
        location: {
          repo: 'myproject/api',
          task: '#145: Add authentication endpoint',
        },
    ...
    return (
      <div className='max-w-3xl mx-auto space-y-4'>
        ...
        <div className='mt-8'>
          <h3 className='text-xl font-semibold text-on-surface mb-4'>Implementation Details</h3>

          <AccordionItem title='API Endpoints' icon={<Code className='h-5 w-5' />}>
          ...
          <AccordionItem title='Security Considerations' icon={<Database className='h-5 w-5' />}>
          ...
          <AccordionItem title='Testing Strategy' icon={<Package className='h-5 w-5' />}>
            <div className='space-y-3'>
              <AccordionItem title='Unit Tests' variant='bordered' size='sm'>
                <p className='text-sm'>
                  Testing authentication service logic, password hashing, token generation
                </p>
              </AccordionItem>
              <AccordionItem title='Integration Tests' variant='bordered' size='sm'>
                <p className='text-sm'>Testing API endpoints with mocked database</p>
              </AccordionItem>
              <AccordionItem title='E2E Tests' variant='bordered' size='sm'>
                <p className='text-sm'>
                  Full authentication flow from login to protected route access
                </p>
              </AccordionItem>
            </div>
          </AccordionItem>
...
export const WithProcessing: Story = {
  render: () => {
    ...
    return (
      <div className='max-w-3xl mx-auto space-y-4'>
        {messages.map(message => (
          <BotReplyBox key={message.id} status={message.status} content={message.content} />
        ))}

        {isProcessing && (
          <div className='rounded-xl bg-surface border border-outline shadow-sm p-8'>
            <BusyIndicator variant='dots' size='lg' message='Generating detailed analysis...' />
          </div>
        )}
      </div>
    );
