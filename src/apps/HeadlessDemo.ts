...
export class HeadlessDemo {
  ...
  async demoMobileEmulation(): Promise<void> {
    try {
      ...
      await fs.writeFile(
        path.join(this.resultsDir, 'mobile-screenshot.png'),
        mobileScreenshot
      );
  ...
  private async takeVisualScreenshots(pageId: string): Promise<void> {
    try {
      ...
      await fs.writeFile(
        path.join(this.resultsDir, 'visual-fullpage.png'),
        fullPageScreenshot
      );
      ...
      await fs.writeFile(
        path.join(this.resultsDir, 'visual-viewport.png'),
        viewportScreenshot
      );
      ...
      await fs.writeFile(
        path.join(this.resultsDir, 'visual-clipped.png'),
        clippedScreenshot
      );
  ...
  private generateRecommendations(results: any[]): string[] {
    ...
    const avgPerformance = results.reduce((sum, r) => sum + r.performance.score, 0) / results.length;
    const avgAccessibility = results.reduce((sum, r) => sum + r.accessibility.score, 0) / results.length;
