...
export class ConfigManager {
  ...
  private loadFromEnvironment(): Partial<HeadlessConfig> {
    return {
      ...
      optimization: {
        ...
        optimizationLevel:
          (process.env.OPTIMIZATION_LEVEL as 'basic' | 'standard' | 'aggressive') || 'standard',
        enabledRules: process.env.OPTIMIZATION_ENABLED_RULES
          ? process.env.OPTIMIZATION_ENABLED_RULES.split(',')
          : [],
        disabledRules: process.env.OPTIMIZATION_DISABLED_RULES
          ? process.env.OPTIMIZATION_DISABLED_RULES.split(',')
          : [],
  ...
  private mergeConfigs(
    ...
    envConfig: Partial<HeadlessConfig>
  ): HeadlessConfig {
    return {
  ...
  private validateConfig(config: HeadlessConfig): void {
    ...
    if (
      config.optimization.maxConcurrentOptimizations < 1 ||
      config.optimization.maxConcurrentOptimizations > 10
    ) {
