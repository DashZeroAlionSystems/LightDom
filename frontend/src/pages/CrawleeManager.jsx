...
const sanitizeSeederPayload = payload =>
  JSON.parse(
...
const normalizeCrawlerRecord = crawler => {
  ...
  const normalizedTags = Array.isArray(crawler.tags)
    ? crawler.tags
    : safeParseJSON(crawler.tags, []);
...
const CrawleeManager = () => {
  ...
  const createSeederServiceForCrawler = async (crawler, options = {}) => {
    ...
    const resolvedInstanceId =
      instanceId || normalizedCrawler.seeder_service_id || `seeder_${normalizedCrawler.id}`;

    const finalSeedUrls = Array.isArray(seedUrls) ? seedUrls : [];
    const finalKeywords =
      Array.isArray(keywords) && keywords.length > 0 ? keywords : normalizedTags;
  ...
  const handleSubmitCrawler = async formValues => {
    ...
    const seedUrls =
      typeof seedUrlsRaw === 'string'
        ? seedUrlsRaw
            .split(/\r?\n/)
            .map(url => url.trim())
            .filter(Boolean)
        : Array.isArray(seedUrlsRaw)
          ? seedUrlsRaw
          : [];
    ...
    const hideMessage = message.loading(
      modalMode === 'create' ? 'Creating crawler...' : 'Updating crawler...',
      0
    );
    ...
    try {
      if (modalMode === 'create') {
        ...
        if (seederResult?.seederId) {
          const seederMessage = seederResult.created
            ? ' and seeder provisioned'
            : ' and seeder ready';
        ...
        const shouldProvisionSeeder =
          !editingCrawler.seeder_service_id ||
        ...
        if (shouldProvisionSeeder) {
          const updatedCrawlerResponse = await axios.get(
            `/api/crawlee/crawlers/${editingCrawler.id}`
          );
      ...
      render: (features = []) => (
        <Space wrap>
          {features.map(feature => (
            <Tag key={feature} color='geekblue'>
              {feature}
            </Tag>
          ))}
        </Space>
