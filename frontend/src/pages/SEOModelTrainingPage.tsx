...
export const SEOModelTrainingPage: React.FC = () => {
  ...
  const getStatusBadge = (status: string) => {
    ...
    return (
      <span
        className={`inline-flex items-center gap-1 px-2 py-1 text-xs font-medium rounded-md border ${badge.color}`}
      >
        {badge.icon}
        {status}
      </span>
    ...
    return (
      <div className='p-6'>
        <AsyncStateLoading className='min-h-[40vh]'>
          Preparing SEO model training data…
        </AsyncStateLoading>
      </div>
    );
  }
  ...
  return (
    <div className='relative space-y-8 p-6'>
      ...
      <WorkflowPanel
        ...
        description='Adjust hyperparameters and targets before launching a new training job.'
      >
        <WorkflowPanelSection>
          <div className='grid grid-cols-1 gap-6 md:grid-cols-2'>
            ...
            <SelectField
              ...
              onChange={value =>
                setTrainingConfig({
                  ...trainingConfig,
                  algorithm: value as TrainingConfig['algorithm'],
                })
              }
              options={[
            ...
            <SelectField
              ...
              onChange={value =>
                setTrainingConfig({
                  ...trainingConfig,
                  targetMetric: value as TrainingConfig['targetMetric'],
                })
              }
              options={[
      ...
      <WorkflowPanel
        ...
        description='Monitor versioned models, deployment status, and evaluation metrics.'
      >
        {modelsLoading ? (
            ...
            {models?.map(model => (
              ...
              >
                ...
                <div className='grid grid-cols-2 gap-4 md:grid-cols-4'>
                  <StatBlock
                    label='Accuracy'
                    value={`${((model.accuracy || 0) * 100).toFixed(1)}%`}
                  />
                  <StatBlock
                    label='F1 score'
                    value={`${((model.f1_score || 0) * 100).toFixed(1)}%`}
                  />
                  <StatBlock
                    label='Training samples'
                    value={(model.training_samples || 0).toLocaleString()}
                  />
                  <StatBlock
                    label='Deployed'
                    value={
