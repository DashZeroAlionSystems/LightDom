/**
 * CodebaseSearch Component (Frontend)
 * 
  ...
  return (
    <div className={cn('space-y-4', className)}>
      ...
      <div className="relative">
        <div className="flex items-center gap-2 rounded-2xl border-2 border-outline/30 bg-surface-container p-2 focus-within:border-primary transition-colors">
      ...
      {/* Advanced options */}
      {showAdvanced && (
        <div className="flex flex-wrap gap-4 p-4 rounded-xl bg-surface-container border border-outline/20">
          <div className="flex items-center gap-2">
            ...
            <input
              ...
              min={1}
              max={50}
              className="w-16 px-2 py-1 rounded border border-outline/30 bg-surface text-on-surface text-sm"
            />
          </div>
          <div className="flex items-center gap-2">
            ...
            <input
              ...
              max={1}
              step={0.1}
              className="w-20 px-2 py-1 rounded border border-outline/30 bg-surface text-on-surface text-sm"
            />
          </div>
          ...
          {showModelSelector && models.length > 0 && (
            <div className="flex items-center gap-2">
              ...
              <select
                ...
                className="px-2 py-1 rounded border border-outline/30 bg-surface text-on-surface text-sm"
                disabled={isLoading}
              >
                {models.map((model) => (
          ...
          <button
            ...
            className={cn(
              ...
              'border border-outline/30 text-on-surface hover:bg-surface-container-highest',
              'disabled:opacity-50 disabled:cursor-not-allowed'
            )}
          >
        ...
        <div className="space-y-3">
          ...
          <div className="space-y-2">
            {searchResults.map((result) => (
              <div
                key={result.id}
                className="rounded-xl border border-outline/20 bg-surface overflow-hidden transition-shadow hover:shadow-lg"
              >
                {/* Result header */}
                <button
                  onClick={() => toggleExpand(result.id)}
                  className="w-full flex items-center justify-between p-4 cursor-pointer hover:bg-surface-container-lowest text-left"
                >
