...
async function deployDevnet() {
  ...
  try {
    ...
    const pooOutput = execSync(
      'forge create --rpc-url http://localhost:8545 --private-key 0xac0974bec39a17e36ba4a6b4d238ff944bacb478cbed5efcae784d7bf4f2ff80 src/ProofOfOptimization.sol:ProofOfOptimization --constructor-args "0x0000000000000000000000000000000000000000" "0x0000000000000000000000000000000000000000"',
    ...
    const tokenOutput = execSync(
      'forge create --rpc-url http://localhost:8545 --private-key 0xac0974bec39a17e36ba4a6b4d238ff944bacb478cbed5efcae784d7bf4f2ff80 src/DOMSpaceToken.sol:DOMSpaceToken',
      { cwd: 'contracts', encoding: 'utf8' }
    );
    const tokenAddress = tokenOutput.match(/Deployed to: (0x[a-fA-F0-9]{40})/)?.[1];

    console.log('📦 Deploying VirtualLandNFT...');
    const landOutput = execSync(
      'forge create --rpc-url http://localhost:8545 --private-key 0xac0974bec39a17e36ba4a6b4d238ff944bacb478cbed5efcae784d7bf4f2ff80 src/VirtualLandNFT.sol:VirtualLandNFT --constructor-args "DOM Space Land" "DSL"',
      { cwd: 'contracts', encoding: 'utf8' }
    );
