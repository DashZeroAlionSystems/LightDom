...
async function main() {
  ...
  const deploymentInfo = {
    ...
    configuration: {
      ...
      biomeMultipliers: biomeMultipliers.reduce(
  ...
  const deploymentPath = path.join(
    __dirname,
    '../deployments',
    `${deploymentInfo.network}-${deploymentInfo.chainId}.json`
  );
  ...
  console.log(
    `npx hardhat verify --network ${deploymentInfo.network} ${optimizationRegistry.address}`
  );
  console.log(
    `npx hardhat verify --network ${deploymentInfo.network} ${proofOfOptimization.address} "604800"`
  );
